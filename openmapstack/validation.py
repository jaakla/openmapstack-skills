...
class _Validator:
    ...
    def _qgis_layer_crs(self) -> None:
        ...
        Delegates to the public ``qgis.every_layer_declares_crs`` check so
        ``validate`` cannot pass a project that ``verify`` and external
        graders fail. An earlier copy here accepted an authority id alone,
        which QGIS reads back as an invalid CRS it cannot transform.
        """
        result = qgis_checks.every_layer_declares_crs(self.root)
        status = result.status
        if result.data.get("code") in {"no_qgs_document", "no_layers"}:
            # An archive with no layers has nothing to reproject; keep it a
            # warning rather than a CRS failure.
            status = "warning"
        self.add("qgis.layer_crs", status, result.detail, path="project.qgz", **result.data)
